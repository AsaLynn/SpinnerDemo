...
/**
 * 1,转圈位置:开始的位置和停下来的位置要一样
 * 2,对勾的出现要由小变大,处于正中间的位置.
 * <p>
...
public class DashSpinner extends View {
    ...
    private static final float STATE_LINE_STROKE = 15.0f;
    ...
    private static final String TAG = "DashSpinner";
...
//    private float mnIndeterminateStartPosition = 270;
...
//    private DynamicLayout mDynamicLayout = null;
    ...
    public DashSpinner(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            mOuterRingColor = a.getColor(R.styleable.DashSpinner_outerRingColor, context.getResources().getColor(android.R.color.holo_blue_dark));
            mInnerCircleSuccessColor = a.getColor(R.styleable.DashSpinner_innerCircleSuccessColor, context.getResources().getColor(android.R.color.holo_green_light));
            mInnerCircleFailureColor = a.getColor(R.styleable.DashSpinner_innerCircleFailureColor, context.getResources().getColor(android.R.color.holo_red_light));
            mInnerCircleUnknownColor = a.getColor(R.styleable.DashSpinner_innerCircleUnknownColor, context.getResources().getColor(android.R.color.holo_orange_light));
            mArcColor = a.getColor(R.styleable.DashSpinner_arcColor, context.getResources().getColor(android.R.color.white));
            mTextColorFrom = a.getColor(R.styleable.DashSpinner_textColorFrom, context.getResources().getColor(android.R.color.black));
            mTextColorTo = a.getColor(R.styleable.DashSpinner_textColorTo, context.getResources().getColor(android.R.color.white));
        ...
        //mTextPaint.setTypeface(Typeface.create("sans-serif-light", Typeface.BOLD));
...
//                nDrawRadius = mnInnerCircleRadius;
    ...
    private void drawArc(Canvas canvas) {
        ...
        if ((mnIndeterminateStartPosition > CIRCULAR_FACTOR) || (mnIndeterminateStartPosition < 0)) {
            //if ((mnIndeterminateStartPosition % 270 == 0) || (mnIndeterminateStartPosition < 0)) {
            mnIndeterminateStartPosition = 0;
            //mnIndeterminateStartPosition = 270;
            mTurnCount++;
        }
        ...
        float nRingBoundaryInner = mnRingRadius;
        ...
        //mArcRect.set(mnViewCenter, mnViewCenter , mnViewCenter , mnViewCenter );
        ...
        //DashSpinner
        Log.i(TAG, "--mnIndeterminateStartPosition: " + mnIndeterminateStartPosition + "--mnArcLength:" + mnArcLength);
        canvas.drawArc(mArcRect, mnIndeterminateStartPosition, mnArcLength, false, mPaint);
        //}

        //mnIndeterminateStartPosition += 1;
    }
    ...
    public void start() {
        mTurnCount = 0;
        this.post(runnableSuccess);
    }

    private int mTurnCount;
    private Handler mHandler = new Handler();
    private Runnable runnableSuccess = new Runnable() {
        @Override
        public void run() {
            //setProgress();
            mnIndeterminateStartPosition += (1 * mnStartSpeed * 0.1);
            postInvalidate();
            if (mIsLoop) {
                long mSpeed = (long) (100 / mnStartSpeed);
